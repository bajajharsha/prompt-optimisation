...
class OpenAIService:
    ...
    def __init__(self):
        self.api_key = os.getenv("OPENAI_API_KEY") or os.getenv("openai_api_key")
        if not self.api_key:
            raise ValueError("OpenAI API key not found. Please set OPENAI_API_KEY or openai_api_key environment variable.")
    ...
    async def completions(
        ...
        **kwargs
        ...
        if not model_name:
            raise ValueError("model_name is required for OpenAI service")
    ...
    async def _log_usage(self, response_data: Dict[str, Any], model: str, operation: str):
        ...
        try:
            ...
            log_entry = {
                ...
                "file_name": "/Users/harshabajaj/Desktop/PERSONAL_PROJECT/Backend/fastapi_optimization_system/app/services/openai_service.py",
